 ...
 */
UCLASS(abstract)
class CHUANGSHI_API AEffectEmitter : public AEmitter
{
...
protected:
	...
	void SetMeshComponentCollision(bool State, ECollisionResponse CollisionResponse = ECollisionResponse::ECR_Overlap);

	///获取粒子绑定初始化对象
	USceneComponent* GetBindComponent();
...
public:
	...
	*
	*/
	void SetEmitterCollisionData(const int32 ProxyID);

	void ResetTarget(AActor* Target1, AActor* Target2);
protected:
	...
	void Stop(bool notDelay = false, bool notDelayDestroyChildren = true, bool bPlayDeathParticle = true, bool bAddParticleBufferPool = true);

	///如果有延时销毁则会调用此方法，处理光效Stop后要做的额外的事情
	virtual void DelayDestroySetting();

	///销毁
	void DestroyEffect(bool bAddParticleBufferPool = true);
...
protected:
	...
	///空的场景组件，主要处理不直接对当前粒子和粒子组件直接设置位置坐标
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
		USceneComponent* SceneTransformComp;

	///声音组件
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	class UCS3AudioComponent* AudioComponent;
	...
	///残留效果网格物体
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UStaticMeshComponent* MotionBlurComponent;
...
private:
	FTimerHandle DelayPlayHandle; //延时播放光效
/*-----------------粒子管理器传过来的属性--------------
*
*/
...
private:
	...
	FTimerHandle BulletBuffEventHandle;///<子弹射线碰撞事件定时器

	int32 EmitterCollisionProxyId = -1;///<粒子碰撞目标ID CST-11238   
/*-----------------GetAttributes--------------
*
*/
