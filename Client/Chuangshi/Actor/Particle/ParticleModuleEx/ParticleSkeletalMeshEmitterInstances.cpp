...
void FParticleSkeletalMeshEmitterInstance::KillParticles()
{
	...
	if (ActiveParticles > 0)
	{
		...
		for (int32 i = ActiveParticles - 1; i >= 0; i--)
		{
			...
			//UE_LOG(LogTemp, Log, TEXT("#############################粒子生命时间为：%f"), Particle.RelativeTime);
...
// 				USkeletalMeshComponent* Comp = (USkeletalMeshComponent*) PayloadData->MeshId;
...
uint64 FParticleSkeletalMeshEmitterInstance::FindInactiveParticle()
{
	if (ActiveParticles > 0)
	{
		for (int32 j = Meshes.Num() - 1; j >= 0; j--)
		{
			USkeletalMeshComponent* MeshComponent = Meshes[j];
			uint64 MeshId = (uint64)MeshComponent;
			if (MeshId == 0)
				continue;
			bool actived = false;
			for (int32 i = ActiveParticles - 1; i >= 0; i--)
