...
#include "Actor/Particle/ParticleModuleEx/ParticleDecalEmitterInstances.h"
// UE4
...
uint64 FParticleDecalEmitterInstance::FindInactiveParticle()
{
	if (ActiveParticles > 0)
	{
		for (int32 j = Decals.Num() - 1; j >= 0; j--)
		{
			UDecalComponent* DecalComponent = Decals[j];
			uint64 Id = (uint64)DecalComponent;
			if (Id == 0)
				continue;
			bool actived = false;
			for (int32 i = ActiveParticles - 1; i >= 0; i--)
