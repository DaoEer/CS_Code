...
// 	if (Inst->Decals.Num() >= MaxParticleNum)
// 	{
// 		Id = Inst->FindInactiveParticle();
// 	}
// 	else
	{
...
void UParticleModuleDecal::UpdateMaterial(UDecalComponent* decalComponent, const FBaseParticle& Particle, const FParticleEmitterInstance* Owner)
{
	if (!decalComponent)
		return;
	...
	UMaterialInterface * Mat = decalComponent->GetDecalMaterial();

	UMaterialInstanceDynamic * dyMat = Cast<UMaterialInstanceDynamic>(Mat);
	if (dyMat)
	{
...
void UParticleModuleDecal::Update(FParticleEmitterInstance* Owner, int32 Offset, float DeltaTime)
{
	...
	BEGIN_UPDATE_LOOP;
	{
		...
		if (Data.DecalId != 0)
		{
			...
			UpdateMaterial(decalComponent, Particle, Owner);
		}
	}
	END_UPDATE_LOOP;	
}
