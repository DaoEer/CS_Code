	...
	if (bMoving)
	{
		...
		else
		{
			...
			if (Emitter)
			{
				...
				CS3_Display(CS3DebugType::CL_Undefined, TEXT("the actor end motion,particle id:%s"), *Emitter->GetEffectID());
			}
		}
	}
}

void ABaseSplineAsset::SceneComponentMoveOnSpline(USceneComponent * Target, bool bHaveOffset, float DeltaTime)
{
	if (bMoving)
	{
		if (SetDistance(DeltaTime))
...
void ABaseSplineAsset::SetTargetTransform(UParticleSystemComponent* Target, bool bHaveOffset)
{
	...
	FVector _CS = SplineComponent->GetScaleAtDistanceAlongSpline(CurrentDistance);
	FTransform _CT = FTransform(_CR, _CFL, _CS);
	if (bHaveOffset)
	{
		Target->SetWorldLocationAndRotation(_CFL, _CT.TransformRotation(Offset.Quaternion()).Rotator());
		Target->SetWorldScale3D(_CS);
	}
	else
	{
		Target->SetWorldTransform(_CT);
	}
}

void ABaseSplineAsset::SetTargetTransform(USceneComponent * Target, bool bHaveOffset)
{
	while (CurrentDistance - LastDistance > 60)
	{
		LastDistance += 60;
