...
void UResourceManager::RequestAsyncItemIconLoad(const UObject* UserObject, const TSoftObjectPtr<UObject>& TargetToStream, FResourceLoaderIconDelegate DelegateToCall, int32 itemid, RESOURCE_LOAD_PRIORITY_TYPE type, FVector Position, bool bManageActiveHandle )
{
	RequestAsyncLoad(UserObject, TargetToStream.ToSoftObjectPath(), FStreamableDelegate::CreateLambda([DelegateToCall, itemid, TargetToStream]() {
		DelegateToCall.ExecuteIfBound(itemid, TargetToStream.Get() );
	}), type, Position, bManageActiveHandle);
}
...
void UResourceManager::FindLoadFaileAssetPathToLog(TArray<FStringAssetReference> LoadPartArray)
{
	for (int32 i = 0; i < LoadPartArray.Num(); i++)
	{
		...
		if (!itemObj)
		{
			CS3_Warning(TEXT("-->Null Pointer error:UResourceManager::FindLoadFaileAssetPathToLog : itemObj!"));
		}
	}
}
...
void UResourceManager::CheckMultipleAssetFileIsExist(TArray<FStringAssetReference> LoadPartArray, int32 EntityID, FString ModelID)
{
	for (int32 i = 0; i < LoadPartArray.Num(); i++)
	{
		...
		{
			CS3_Warning(TEXT("RequestAsyncLoad File is not exist!!!! Asset[path=%s] EntityID=%d ModelID=%s assetName=%s"), *TargetToStream.ToString(), EntityID, *ModelID, *TargetToStream.GetAssetName());
		}
	}
}
...
TAsyncLoadPriority UResourceManager::GetAsyncLoadPriority(RESOURCE_LOAD_PRIORITY_TYPE type, FVector Position)
{
	...
	AServerCharacter* ServerCharacter = UGolbalBPFunctionLibrary::GetCS3PlayerCharacter();
...
void UResourceManager::ReleaseHandlesByObject(const void * UserObject)
{
	...
	{
		//保证shutdown的正确性
		CS3_Display(CS3DebugType::CL_Undefined, TEXT("UResourceManager is NULL"));
		return;
	}
	if (UserObject)
	{
