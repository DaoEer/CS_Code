...
/*
* 文件名称：ParticleManager.h
* 功能说明：粒子光效管理类，用于通过ID及目标对象创建光效实例，并通过回调函数返回
* 文件作者：luocheng
* 目前维护：zhangfudong
* 创建时间：2016-08-31
*/
...
// 粒子管理核心数据结构
USTRUCT()
struct CHUANGSHI_API FInternal_EffectData
{
...
struct FPARTICLE_DIAPLAY_CONFIG : public FCSV_TABLE_ROW_BASE
{
	GENERATED_USTRUCT_BODY()
public:
	///最大数量（个）
...
class CHUANGSHI_API UParticleManager : public UCS3Object, public FTickableGameObject
{
	GENERATED_BODY()

public:
	// 绑定到配置改变那边的事件
	void UpdateParticleDiaplayConfig(FVIDEOSETTING_DATA& Data);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Particle")
	void Stop(class AEffectEmitter* Effect, bool notDelay = false, bool notDelayDestroyChildren = false, bool bPlayDeathParticle = true, bool bAddParticleBufferPool = true);

	UFUNCTION(BlueprintCallable, Category = "Particle")
	void StopEffectByID(FString EffectID);

	UFUNCTION(BlueprintCallable, Category = "Particle")
	void StopEffectByUID(int32 UID, bool notDelay = false, bool notDelayDestroyChildren = false, bool bPlayDeathParticle = true, bool bAddParticleBufferPool = true);
	...
	/// 通过UID设置粒子的有效性
	void SetEffectValidByID(int32 UID,bool bIsValid);
	...
	void LoadParticleAsset(FString EffectID);

	void RemoveInvalidEffect(int32 UID);

protected:
	///蓝图更新粒子显示的规则表
	UFUNCTION(BlueprintNativeEvent, Category = "Particle")
	void UpdateParticleDisplayConfig();
...
public:
	...
	//当前的粒子个数
	UPROPERTY()
	FPARTICLE_DIAPLAY CurrentParticleDisplayConfig;
