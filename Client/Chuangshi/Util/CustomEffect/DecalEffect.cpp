...
#include "GameDevelop/CS3GameInstance.h"
#include "Component/DecalComponentEX.h"
...
void ADecalEffect::BeginPlay()
{
	...
	UUECS3GameInstance::Instance->CustomTimerManager->SetTimer(m_TimerHandle, this, &ADecalEffect::_CreateDecal, 1.0 / m_EmitterData->SpawnRate, true, m_EmitterData->Delay);
	...
	if (m_EmitterData->EmitterLoops != 0)
	{
		...
		UUECS3GameInstance::Instance->CustomTimerManager->SetTimer(TimerHandle, this, &ADecalEffect::_StopEmit, m_Lifetime - m_EmitterData->Lifetime - m_EmitterData->SpawnRate, false);
	}
}
...
void ADecalEffect::Tick(float DeltaTime)
{
	...
	if (!m_EmitterData)
	{
		CS3_Warning(TEXT("-->Null Pointer error:ADecalEffect::Tick: m_EmitterData!"));
		return;
	}
	//在第一次Tick生成贴花组件
	...
	for (int i = 0; i < m_DecalComps.Num(); i++)
	{
		...
		if (par->Lifetimer < par->Lifetime)
		{
			...
			fDynamic = m_EmitterData->DynamicParams.Num() > 0 ? m_EmitterData->DynamicParams[0].GetValue(F) : 0.0f;
			...
			//fDynamic = m_EmitterData->DynamicParams.Num() > 0 ? m_EmitterData->DynamicParams[0].GetValue[F] : 0.0f;
			if (m_EmitterData->DynamicParams.Num() > 0)
				fDynamic = m_EmitterData->DynamicParams[0].IsCreated() ? m_EmitterData->DynamicParams[0].GetValue(F) : 0.0f;
			else
				fDynamic = 0.0f;
...
void ADecalEffect::PauseDecal()
{
	...
	UUECS3GameInstance::Instance->CustomTimerManager->PauseTimer(TimerHandle);
	UUECS3GameInstance::Instance->CustomTimerManager->PauseTimer(m_TimerHandle);
}
...
void ADecalEffect::_CreateDecal()
{
	...
	if (!decal)
	{
		CS3_Warning(TEXT("-->Null Pointer error:ADecalEffect::_CreateDecal: decal!"));
		return;
	}
...
// 		return true;
// 	}
	for (int32 i = 0; i < ParamModule->DynamicParams.Num(); i++)
	{
		DynamicParams.Add(ParamModule->DynamicParams[i].ParamValue);
	}
