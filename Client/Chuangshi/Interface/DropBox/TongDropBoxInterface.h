...
#include "TongDropBoxInterface.generated.h"

/*
* 文件名称：TongDropBoxInterface.h
* 功能说明：
* 文件作者：qiurunan
* 目前维护：qiurunan
* 创建时间：2020-11-13
...
class CHUANGSHI_API UTongAllocationItemData : public UCS3Object
{
	GENERATED_BODY()
public:
	///< dbid
 ...
 */
UCLASS(BlueprintType)
class CHUANGSHI_API UTongDropBoxInterface : public UNPCBaseObjectInterface
{
	GENERATED_BODY()
	typedef UNPCBaseObjectInterface Supper;
	CS3_DECLARE_INTERFACE_MAP()
public:
	UTongDropBoxInterface();
	~UTongDropBoxInterface();
	static FString GetName() { return TEXT("TongDropBoxInterface"); } //获取接口名字
	static FString GetClassPath() { return TEXT("Class'/Script/Chuangshi.TongDropBoxInterface'"); } //获取蓝图对象路径
	...
	bool isCanOpenBox();
	///宝箱物品分配是否是某种分配方式
	bool isAllocationType(TONG_PICKUP_TYPE type);

	///更新宝箱可见性
	UFUNCTION(BlueprintNativeEvent, Category = "TongDropBoxInterface")
		void onUpdateDropBoxModelVisible();

	void onOpenDropbox();
	
	///请求获取宝箱数据
	UFUNCTION(BlueprintCallable, Category = "TongDropBoxInterface")
		void ReqTongDropBoxData();
	//宝箱物品数据
	...
	/// 某人取得了某物品(帮主分配)
	UFUNCTION(BlueprintNativeEvent, Category = "TongDropBoxInterface")
		void onPickUpByLeader(const FString& uid, const FString& playerDBID, const int32& targetID);
	...
	/**
	*BlueprintCallable,获取所有帮会分配物品数据
	*
	...
	*/
	...
	void FlyTargerOverEvent();//飞行光效到达通知

	//打开帮主分配界面
	void openTongLeaderPickUpWnd();

	UFUNCTION(BlueprintCallable, Category = "TongDropBoxInterface")
		void OnRep_isCanOpen();

public:
	///参与宝箱分配的玩家 {DBID:NAME}
	UPROPERTY(BlueprintReadOnly, Category = "TongDropBoxInterface")
	...
	///未进行分配的物品{uid:item} 帮主进行分配的物品
	UPROPERTY(BlueprintReadOnly, Category = "TongDropBoxInterface")
		TMap<FString, UItemBase*> notAllocationItems;

	///帮主已分配的物品 {dbid:data}
	UPROPERTY()
		TMap<FString, UTongAllocationItemData*> leaderHasAllocationItems;
};
