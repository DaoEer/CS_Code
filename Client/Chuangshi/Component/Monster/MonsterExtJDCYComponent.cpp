...
#include "MonsterExtJDCYComponent.h"
...
UMonsterExtJDCYComponent::UMonsterExtJDCYComponent()
{
	//bWantsBeginPlay = false;
	PrimaryComponentTick.bCanEverTick = true;
}
KBEngine::Entity* UMonsterExtJDCYComponent::GetMEntity()
{
...
void UMonsterExtJDCYComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		//CS3_Display(CS3DebugType::CL_Undefined, TEXT("---------------------------UMonsterExtJDCYComponent::TickComponent--------------"));
		return;
	}
	if (GetMEntity())
	{
...
MONSTER_MOVE_TYPE UMonsterExtJDCYComponent::GetCurMonsterMoveType()
{
	return CurrMoveType;
}

MONSTER_MOVE_TYPE UMonsterExtJDCYComponent::GetLastMonsterMoveType()
{
	return LastMoveType;
}

void UMonsterExtJDCYComponent::ChaseTarget()
{
	if (!IsValid(GetOwner()) || !IsValid(Cast<AMonsterCharacter>(GetOwner())) || Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity() == nullptr) return;
	...
	if (targetID)
	{
		...
		TWeakObjectPtr<UMonsterExtJDCYComponent>DelayAIComponentPtr(this);
		TraceTargetActor(TargetActor, Distance, FTraceCallback::CreateLambda([DelayAIComponentPtr](bool IsSuccess)
		{
			...
			{
				UMonsterExtJDCYComponent* ThisPtr = DelayAIComponentPtr.Get();
...
void UMonsterExtJDCYComponent::AttackTarget()
{
	//攻击玩家
	if (!IsValid(GetOwner()) || !IsValid(Cast<AMonsterCharacter>(GetOwner())) || Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity() == nullptr) return;
	KBEngine::Entity* Entity = Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity();
...
int32 UMonsterExtJDCYComponent::GetTargetID()
{
	//寻找一定范围内的可攻击目标
...
void UMonsterExtJDCYComponent::StartWander()
{
	if (!IsValid(GetOwner()) || !IsValid(Cast<AMonsterCharacter>(GetOwner())) || Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity() == nullptr) return;
	KBEngine::Entity* Entity = Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity();
	...
	if (WanderPosition.IsZero())
	{
		WanderPosition = Cast<AMonsterCharacter>(GetOwner())->SpawnPosition;
	}
	FVector targetPosition = WanderPosition + FVector(FMath::Sin(PI / 180.0 * theta)*radius, FMath::Cos(PI / 180.0*theta)*radius, 0); 
	if (FVector::Dist(GetOwner()->GetActorLocation(), targetPosition) < 1.0)
	{
		SetMonsterMoveType(MONSTER_MOVE_TYPE::None);
		return;
	}
	WanderPosition = targetPosition;
	TWeakObjectPtr<UMonsterExtJDCYComponent>DelayAIComponentPtr(this);
...
void UMonsterExtJDCYComponent::BeHitBack()
{
	SetMonsterMoveType(MONSTER_MOVE_TYPE::HitBack);
}

void UMonsterExtJDCYComponent::GoBack()
{
	if (!IsValid(GetOwner()) || !IsValid(Cast<AMonsterCharacter>(GetOwner())) || Cast<AMonsterCharacter>(GetOwner())->GetSelfEntity() == nullptr) return;
...
void UMonsterExtJDCYComponent::TraceTargetActor(const AActor* TargetActor, float NearbyRange /*= 0.0f*/, FTraceCallback TraceCallback /*= FTraceCallback()*/, bool IsStopMove/* = true*/, FTraceCallback TraceStopMoveCallback /*= FTraceCallback()*/)
{
...
void UMonsterExtJDCYComponent::WanderTargetPostion(const FVector& TargetPostion, float NearbyRange /*= 0.0f*/, FTraceCallback TraceCallback /*= FTraceCallback()*/, bool IsStopMove/* = true*/, FTraceCallback TraceStopMoveCallback /*= FTraceCallback()*/)
{
...
void UMonsterExtJDCYComponent::BackTargetPostion(const FVector& TargetPostion, float NearbyRange /*= 0.0f*/, FTraceCallback TraceCallback /*= FTraceCallback()*/, bool IsStopMove /*= true*/, FTraceCallback TraceStopMoveCallback /*= FTraceCallback()*/)
{
...
void UMonsterExtJDCYComponent::ClearTraceData()
{
	TraceTargetInfo.TraceTargetType = ENUM_TRACE_TARGET_TYPE::TraceTargetActor;
...
void UMonsterExtJDCYComponent::SetMonsterMoveType(MONSTER_MOVE_TYPE MonsterMoveType)
{
	//set move type
