...
#include "MAI_Ser_UpdateGuardTargetPos.h"
...
void UMAI_Ser_UpdateGuardTargetPos::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
				...
				if (ControlRole)
				{
					...
					if (IsValid(ControlRoleActor))
					{
						if (FVector::Dist2D(ControlRoleActor->GetActorLocation(), LastPlayerPos)>10.f)
						{
							...
							Interface->ResetGuardFollow(pCMonster->ID());
						}
