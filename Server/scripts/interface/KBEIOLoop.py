...
class KBEIOLoop(IOLoop):
	...
	def stop(self):
		...
		for timeout in self._timeouts:
			KBEngine.delTimer( timeout.kbe_timer_id )
	...
	def add_callback(self, callback, *args, **kwargs):
		...
		if list_empty:
			# 使用timer来模拟下一次的IO循环
			KBEngine.addTimer( 0.01, 0.0, self.kbe_onCallback )
	...
	def call_at(self, deadline, callback, *args, **kwargs):
		...
		timeout.kbe_timer_id = KBEngine.addTimer( deadline - self.time(), 0.0, timeout.kbe_on_callback )
