...
class LingMaiDatas( dict ):
	...
	def add( self, player, id, name, camp, campStr, level ):
		...
		for _id, datas in self.items():
			player.client.CLIENT_AddLingMaiMember( _id, datas["name"], datas["camp"] )
			if _id != id:
				otherPlayer = KBEngine.entities.get( _id )
				if otherPlayer:
					otherPlayer.client.CLIENT_AddLingMaiMember( id, name, camp )
	...
	def remove( self, id ):
		...
		if id in self:
			...
			if player and player.client:
				player.client.CLIENT_OnLeaveLingMai()
	...
	def kill( self, diePlayerID, killerID ):
		...
		if diePlayerID in self:
			if not killerID in self:			
				...
				if p and p.isEntityFlag( csdefine.ENTITY_FLAG_VEHICLEPET ):	# 幻兽杀死玩家
					...
					for data in self.values():
						...
						if player and player.client:
							player.client.CLIENT_OnKillLingMai( diePlayerID, killerID )
					return
			...
			dieEntity = KBEngine.entities.get(diePlayerID, None)
			if killer:
				killer.getCurrentSpace().onMemberKillPlayer( killer, dieEntity )
			for data in self.values():
				player = KBEngine.entities.get( data["id"] )
				if player and player.client:
					player.client.CLIENT_OnKillLingMai( diePlayerID, killerID )
	...
	def occupyPagoda( self, id ):
		"""占领塔基"""
		self[id]["occupy"] += 1
		
	def occupyLingMai( self, idList ):
		"""杀死boss，占领灵脉"""
		for id in idList:
			if id in self:
				self[id]["occupy"] += 1
		
	def grabGourd( self, id, camp ):
		"""抢夺葫芦"""
		if id in self:
			self[id]["grab"] += 1
	...
	def isSameType( self, obj ):
		return isinstance( obj, LingMaiDatas )

g_lingMaiDatas = LingMaiDatas()