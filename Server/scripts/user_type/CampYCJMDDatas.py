...
class CampYCJMDDatas( dict ):
	...
	def add( self, player, id, name, camp, campStr, level ):
		""""""
		self[id] = {}
		self[id]["id"] = id
		self[id]["name"] = name
		...
		self[id]["campStr"] = campStr
		self[id]["isLeave"] = 0 #是否出局
		self[id]['level'] = level
		for _id, datas in self.items():
			player.client.CLIENT_AddCampYCJMDMember( _id, datas["name"], datas["camp"] )
			if _id != id:
				otherPlayer = KBEngine.entities.get( _id )
				if otherPlayer:
					otherPlayer.client.CLIENT_AddCampYCJMDMember( id, name, camp )
	...
	def remove( self, id ):
		...
		if id in self:
			...
			if player and player.client:
				player.client.CLIENT_OnLeaveCampYCJMD()
	...
	def kill( self, diePlayerID, killerID ):
		...
		if diePlayerID in self:
			...
			for data in self.values():
				...
				if player and player.client:
					player.client.CLIENT_OnKillCampYCJMD( 0, killerID )
			campStr = killerData["campStr"]
			dieCampStr = diePlayerData["campStr"]		
			# 显示击杀消息
			for data in self.values():
				player = KBEngine.entities.get( data["id"] )
				if player and player.client:
					player.statusMessage( csstatus.CAMP_YCJMD_KILL, campStr, killerData["name"], dieCampStr, diePlayerData["name"] )
	...
	def dead(self, diePlayerID):
		"""
		死亡
		"""
		if diePlayerID in self:
			self[diePlayerID]["dead"] += 1			# 被怪物杀死
			self[diePlayerID]["continuityKill"] = 0
			for data in self.values():
				player = KBEngine.entities.get( data["id"] )
				if player and player.client:
					player.client.CLIENT_OnKillCampYCJMD( diePlayerID, 0 )
	...
	def checkAssists( self, id ):
		...
		if temp:
			for data in self.values():
				player = KBEngine.entities.get( data["id"] )
				if player and player.client:
					player.client.CLIENT_OnAssistsLingMai( temp )

	def sendRewardMail(self, winnerCamp, loserRewardMailID, winnerRewardMailID):
		for value in self.values():
			if value["camp"] == winnerCamp:
				g_mailContentDataLoader.sendSpaceReward(winnerRewardMailID, value["name"])
			else:
				g_mailContentDataLoader.sendSpaceReward(loserRewardMailID, value["name"])

	
	#@classmethod
	def isSameType( self, obj ):
		return isinstance( obj, CampYCJMDDatas )

g_campYCJMDDatas = CampYCJMDDatas()