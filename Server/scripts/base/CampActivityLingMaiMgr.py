...
class CampActivityLingMaiMgr( MgrPersistentObject.MgrPersistentObject ):
	"""
	灵脉战场管理器
	"""
	def __init__( self ):
	...
	def register( self ):
		...
		注册
		"""
		taskEvents = { "CampLMZCCopyMgr_Notice":"onActiveNotice",
						"CampLMZCCopyMgr_Start":"onActiveStart",
						"CampLMZCCopyMgr_End":"onActiveEnd"
		 }
...
#		KST.g_baseAppEntity.globalWold( csstatus.ACTIVITY_CAMP_LING_MAI_NOTICE, "" )
	...
	def onActiveStart( self, cmd ):
		...
		活动开始
		"""
		KBEngine.globalData["LingMaiBattle"] = 1
...
#		KST.g_baseAppEntity.globalWold( csstatus.ACTIVITY_CAMP_LING_MAI_END, "" )
		KBEngine.globalData["SpaceManager"].remoteCallDomain(  Const.LMZC_SPACE_SCRIPTID, "activityEnd", () )
	...
	def signIn( self, playerMB, camp, level ):
		...
		登记报名
		"""
		if not KBEngine.globalData.get( "LingMaiBattle", 0 ):return
		...
		playerMB.client.CLIENT_OnEnterLingMaiMatchQueue()
		if len( queueDict[levelSec] ) >= Const.CAMP_LING_MAI_MATCH_NUM:
	...
	def lingmai_onPlayerLoginOff( self, playerMB, camp, level ):
		"""
		define method
		玩家下线
		"""
	...
	def _enterReadyQueue( self, camp, levelSec ):
		...
		for playerMB in queue:
			playerMB.client.CLIENT_OnEnterLingMaiReadyQueue()
	...
	def _enterTelPortQueue( self, queue ):
		...
		self._key += 1
		for playerMB in queue:
			playerMB.client.CLIENT_OnEnterLingMaiTelPortQueue( self._key )
	...
	def beginEnterLingMai( self, playerMB, key ):
		"""
		define method
		开始进入战场
		"""
	...
	def _addTeleportTimer( self ):
		"""添加准备阶段timer"""
		timer = self.addTimerCallBack( Const.READY_STAGE_TIMER_SECOND, "_teleportTimeOver", ( self._key, ) )
