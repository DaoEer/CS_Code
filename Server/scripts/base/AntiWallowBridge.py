...
class _Receiver( object ) :
	def __init__( self, owner, host ) :
		...
		KBEngine.registerReadFileDescriptor( self.__sock.fileno(), self.__onAccept )
	...
	def __onAccept( self, fd = None ) :
		...
		KBEngine.registerReadFileDescriptor( connect.fileno(), func )
	...
	def __onReceive( self, connect, host, fd = None ) :
		...
		try :
			...
			INFO_MSG( "socket error from '%s': %s!" % ( str( host ), err ) )
		else :
			if data == "" :													# 如果后台没确认回复前就关闭 socket，则会收到一个空串
				INFO_MSG( "connect is closed by '%s'!" % str( host ) )
			else :
		...
		KBEngine.deregisterReadFileDescriptor( connect.fileno() )
		connect.close()
	...
	def close( self ) :
		...
		关闭连接监听
		"""
		KBEngine.deregisterReadFileDescriptor( self.__sock.fileno() )
		self.__sock.close()
...
class _SendConnector( BackgroundTask ) :
	...
	def __onReceive( self, sock, fd = None ) :
		...
		except Exception as err :													# 接收数据失败
			self.__owner.sendData( self.__id, self.__data )						# 重新放入任务列表，等待下一次发送
			INFO_MSG( "receive data from anti-walow server '%s' failed:\n%s " \
				% ( str( self.__host ), err ) )
		else :
			if data == "" :														# 如果后台主动关闭连接，则会收到空一个空串
				INFO_MSG( "connect is closed by '%s'!" % str( self.__host ) )
			else :
	...
	def doBackgroundTask( self, bgTaskMgr ) :
		try :
			...
			if type( err.args ) is tuple and err.args[0] == 111 :			# 如果因为连接繁忙而被拒绝
				...
				INFO_MSG( "connect to anti-walow server '%s' failed:\n%s " \
					% ( str( self.__host ), err ) )
		else :
			# 下面的代码有以下隐患：
			...
			KBEngine.registerWriteFileDescriptor( sock.fileno(), func )
...
# 防沉迷
# --------------------------------------------------------------------
class AntiWallowBridge( MgrPersistentObject ) :
	def __init__( self ) :
	...
	def __sendData( self, data, mark = None ) :
		...
		if self.__isBgServicing :
			...
			INFO_MSG( "anti-wallow server is not running! it must be running before game server start!" )
			if mark:
				id = self.__genRequestID( mark )
				LoaclSever = KBEngine.globalData["AntiWallowMgr"]
				LoaclSever.sendData( id, mark )
	...
	def __respondLogin( self, accName, text ) :
		...
		if olState not in csconst.WALLOW_STATES :
			...
			KBEngine.lookUpEntityByDBID("Role",self._accname_DBID[accName],Functor(self.sendNotifyToRole,accInfo))
	...
	def onNotify( self, data ) :
		...
		if not self.__isBgServicing :
			INFO_MSG( "receive notify message from anti-wallow server: '%s', but it is not effective.\n" + \
	...
	def sendNotifyToRole(self, accInfo, target):
		"""
		获得玩家MailBox回调
		"""
		if target is False:
		...
		if target is True:
			return
		info = ( accInfo["olState"], accInfo["olTime"] )
		target.wallow_onWallowNotify( *info )
	...
	def onServerStart( self ) :
		...
		d = {
			"gname" : _gameName,
			"groupid" : KBEngine.globalData["serverID"] if KBEngine.globalData.has_key( "serverID" ) else "??",
			"ip"	: self.__svrHost[0],
	...
	def onAccountLogin( self, accName, DBID ) :
		"""
		defined.
		角色登录通知
		"""
		...
		self.__sendData( data, accName )
		self._accname_DBID[accName] = DBID
		LoaclSever = KBEngine.globalData["AntiWallowMgr"]
		LoaclSever.onAccountLogin( accName )
	...
	def onAccountLogout( self, accName ) :
		...
		LoaclSever = KBEngine.globalData["AntiWallowMgr"]
		LoaclSever.onAccountLogout( accName )
