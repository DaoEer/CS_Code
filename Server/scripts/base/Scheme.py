...
def calculateWeekDay( year, month, day ):
	...
	#ERROR_MSG("%d:%d:%d" %(year, month, day))
	try:
	...
	for scheme in schemes:
		...
		if scheme == "*":
			...
			if dividend.isdigit():
				...
				return []
			if scheme == "*":						# */2
				ret_val.extend( [ e for e in range( min, max + 1 ) if e % dividend == 0 ] )
				continue
		...
		if "-" in scheme:						# 3-18 or 3-18/3
			...
			if dividend is not None:
				ret_val.extend( [ e for e in range( minV, maxV + 1 ) if e % dividend == 0 ] )
			else:
				ret_val.extend( range( minV, maxV + 1 ) )
		elif scheme.isdigit():					# 10/2, 理论上这个不应该支持，因为它毫无意义
			ret_val.append( int(int( scheme ) / dividend) )
			continue
		else:									# abc/3 ???
...
class SchemeTypeDate( object ):
	...
	def init( self, monthSchemeString, daySchemeString, wdaySchemeString ):
		...
		if self._wdays[0] == 0:
			self._wdays.pop(0)
			if len(self._wdays) == 0 or self._wdays[-1] < 7:
	...
	def find( self, year, month = None, day = None ):
		...
		while True:
			...
			# 如果选择的年月份与当前不相同，那说明当前日期并没有匹配上合适的时间
			# 这种情况下，日参数就必须从1开始寻找
			if yearE != year or monthE != month:
				dayE = 1
...
def test_Scheme():
	...
	assert s.init("20 17 * * 0") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 10, 17, 20,  0 )
	assert s.init("20 17 * * 1") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 11, 17, 20,  0 )
	assert s.init("20 17 * * 2") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 5,  17, 20,  0 )
	assert s.init("20 17 * * 3") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 6,  17, 20,  0 )
	assert s.init("20 17 * * 4") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 7,  17, 20,  0 )
	assert s.init("20 17 * * 5") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 8,  17, 20,  0 )
	assert s.init("20 17 * * 6") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 9,  17, 20,  0 )
	assert s.init("20 17 * * 7") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 10, 17, 20,  0 )
	assert s.init("20 17 * * 7,6,5,4,3,2,1,0") == True
	assert time.localtime( s.calculateNext( 2017, 9, 5, 11, 55 ) )[:6] == ( 2017, 9, 5,  17, 20,  0 )

	assert s.init("00 03 18 12 *") == True
	print( time.localtime( s.calculateNext( 2017, 10, 29, 10, 2 ) )[:6] )
	assert time.localtime( s.calculateNext( 2017, 10, 29, 10, 2 ) )[:6] == ( 2017, 12, 18,  3, 0,  0 )
