...
class PresentChargeManage:
	...
	def takeThings( self, dataType, item_id ):
		...
		if dataType == csconst.PCU_TAKECHARGE or not self.havePresent():	# 如果是充值请求或者列表中没有领取请求
			self.operations.append( takeTypes[dataType]( self.entity, item_id ) )	# 实例一个该类型的操作,并放到队列中
			if not self.currOperation:										# 如果当前没有操作
				self.currOperation = self.operations.pop(0)					# 那么记录该请求实例
				self.currOperation()								# 执行操作
		else:
	...
	def takeThingsFailed( self ):
		...
		# 因此我们需要对这个情况做处理。
		if self.entity is None or Functions.isUnreliable( self.entity ): 
			self.currOperation = None
	...
	def getPresentTypes( self ):
		...
		KBEngine.executeRawDatabaseCommand( sql, self._onGetPresentTypes )

	def _onGetPresentTypes( self, results, rows, insertid, errstr ):
		"""
		获取了该账号拥有的奖励类型数据
...
class TakeThings:
	...
	领取物品的基础类
	"""
	def __init__( self, entity, item_id ):
		"""
		初始化
		"""
	...
	def operationSuccess( self ):
		...
		KBEngine.executeRawDatabaseCommand( sql, self._onOperationSuccess )

	def _onOperationSuccess( self, results, rows, insertid, errstr ):
		"""
...
class TakePresentWithoutID( TakeThings ):
	...
	def __init__( self,  entity, item_id ):
		...
		初始化
		"""
		TakeThings.__init__( self, entity, item_id )
	...
	def __call__( self ):
		...
		if self.item_id != 0:
			...
			% ( self.entity.accountName,self.item_id )
		else:
		...
		KBEngine.executeRawDatabaseCommand( sql, self._onQueryDatas )

	def _onQueryDatas( self, results, rows, insertid, errstr ):
		"""
		获取了需要的数据
		"""
		# 在异步回调的情况下，entity 为 None或已销毁的情况总是有可能发生的，
	...
	def _onOperationSuccess( self, results, rows, insertid, errstr ):
		...
		处理完毕后写日志和清理数据
		"""
		TakeThings. _onOperationSuccess( self, results, rows, insertid, errstr )
...
class TakeCharge( TakeThings ):
	...
	def __call__( self ):
		...
		到数据库中取出需要字段的数据
		"""
		sql = "SELECT sm_transactionID,sm_chargeType,sm_silverCoins,sm_goldCoins,sm_account,id FROM custom_ChargePresentUnite WHERE sm_account = '%s' and sm_type = 3" % ( self.entity.accountName )
		KBEngine.executeRawDatabaseCommand( sql, self._onQueryDatas )
	...
	def _onQueryDatas( self, results, rows, insertid, errstr ):
		...
		if not silverCoinsList and not goldList:			#没有符合要求的元宝数 直接返回
	...
	def operationSuccess( self ):
		...
		释放entity
		"""
		self.entity.statusMessage( csstatus.PCU_CHARGE_SUCCESS )
		TakeThings.operationSuccess( self )
	...
	def _onOperationSuccess( self, results, rows, insertid, errstr ):
		...
		for params in self.thingsParam:
			...
			#except:
			#	g_logger.logExceptLog( GET_ERROR_MSG() )
			sql = "select tbl_Role.sm_level from tbl_Role where tbl_Role.sm_parentDBID = %d and tbl_Role.sm_level = (select MAX(tbl_Role.sm_level) from \
				tbl_Role where tbl_Role.sm_parentDBID = %d );" % (self.entity.parentDBID, self.entity.parentDBID)
			KBEngine.executeRawDatabaseCommand( sql, Functor(self.selectResult, params[0], params[1], params[3] + params[2] ))
			
		TakeThings. _onOperationSuccess( self, results, rows, insertid, errstr )
				
	def selectResult( self, transactionID, creditType, money, result, rows, insertid, errstr ):
		if errstr:
			return
		level = result[0][0]
		#try:
		#	g_logger.creditLog( transactionID, self.entity.parentDBID, time.time(), creditType, money, level )
		#except:
		#	g_logger.logExceptLog( GET_ERROR_MSG() )
