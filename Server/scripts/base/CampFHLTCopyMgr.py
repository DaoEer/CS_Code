...
class CampFHLTCopyMgr( MgrPersistentObject.MgrPersistentObject ):
	"""
	烽火连天管理器
	"""
	def __init__( self ):
	...
	def register( self ):
		...
		注册
		"""
		taskEvents = { "CampFHLTCopyMgr_Notice":"onActiveNotice",
						"CampFHLTCopyMgr_Start":"onActiveStart",
						"CampFHLTCopyMgr_End":"onActiveEnd"
		 }
	...
	def onActiveNotice( self, cmd ):
		...
		活动公告
		"""
		KST.g_baseAppEntity.globalWold( csstatus.ACTIVITY_CAMP_FHLT_NOTICE, "" )
	...
	def onActiveStart( self, cmd ):
		...
		活动开始
		"""
		KBEngine.globalData["CampFHLTBattle"] = 1
		KST.g_baseAppEntity.globalWold( csstatus.ACTIVITY_CAMP_FHLT_START, "" )
	...
	def onActiveEnd( self, cmd ):
		...
		活动结束
		"""
		KBEngine.globalData["CampFHLTBattle"] = 0
		self._key = 0
		for queue in self._matchQueue.values():
			for levelQueue in queue.values():
				for mb in levelQueue:
					mb.onFHLTActivityEnd()
		...
		for timer in self._timer:
			self.popTimer( timer )
		self._timer.clear()
		KST.g_baseAppEntity.globalWold( csstatus.ACTIVITY_CAMP_FHLT_END, "" )
		KBEngine.globalData["SpaceManager"].remoteCallDomain(  Const.FHLT_SPACE_SCRIPTID, "activityEnd", () )

	def signUp( self, playerMB, camp, level ):
		"""
		define method
		登记报名
		"""
		if not KBEngine.globalData.get( "CampFHLTBattle", 0 ):return
		levelSec = int( level / 10 ) * 10
		
		queueDict = self._matchQueue[camp]
		if not levelSec in queueDict:
			queueDict[levelSec] = []
		queueDict[levelSec].append( playerMB )
		for mb in queueDict[levelSec]:
			mb.client.OnEnterFHLTMatchQueue( len( queueDict[levelSec] ) )
		if len( queueDict[levelSec] ) >= Const.CAMP_FHLT_MATCH_NUM:
		...
		if levelSec in self._matchQueue[camp]:
			for mb in self._matchQueue[camp][levelSec]:
				if mb.id == playerMB.id:
					...
					break
			for mb in self._matchQueue[camp][levelSec]:
				mb.client.OnEnterFHLTMatchQueue( len( self._matchQueue[camp][levelSec] ) )
			return
	...
	def _enterReadyQueue( self, camp, levelSec ):
		...
		for playerMB in queue:
			playerMB.client.OnEnterFHLTReadyQueue()
	...
	def beginEnterFHLT( self, playerMB, key ):
		"""
		define method
		开始进入战场
		"""
	...
	def _enterTelPortQueue( self, queue ):
		...
		self._key += 1
		for playerMB in queue:
			playerMB.client.OnEnterFHLTTelPortQueue( self._key )
		self._teleportQueue[self._key] = queue
		self._addTeleportTimer()

	def _addTeleportTimer( self ):
		"""添加准备阶段timer"""
		timer = self.addTimerCallBack( Const.CAMP_FHLT_READY_STAGE_TIMER_SECOND, "_teleportTimeOver", ( self._key, ) )
	...
	def _teleportTimeOver( self, key ):
		...
		if key in self._teleportQueue:
			for mb in self._teleportQueue[key]:
				mb.telportFHLTActivity( key )
			self._teleportQueue.pop(key)

	def fhlt_onPlayerLoginOff( self, playerMB, camp, level ):
		"""
		define method
		玩家下线
		"""
