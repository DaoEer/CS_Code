...
import time, random, socket, struct, urllib, _md5, binascii
...
#KBEngine.globalData["pointCardWord"] = "tempString"
#KBEngine.globalData["pointCardAddr"]					#充值端口 如：http://transfer.gyyx.cn:81/CsServiceTest/CSCarConsignment.asmx
#KBEngine.globalData["pointCardWord"]					#充值校验码
class PointCharge:
	...
	def recharge( self, cardInfo ):
		...
		path = KBEngine.globalData["pointCardAddr"] + "/CSFill?"
		...
		#卡号 + 密码 + 订单号 + 用户名 + 服务器名 + 寄售玩家名字 + 寄售IP + 面值 + KBEngine.globalData["pointCardWord"]
		#print cardNo + cardPwd + str(cardOrderID) + buyerAccount + urllib.quote(cardInfo.serverName.decode("gbk").encode("utf-8")).lower() + salesName + str(salesIP) + str(parValue) + KBEngine.globalData["pointCardWord"]
		hash = _md5.new()
		hash.update( cardNo + cardPwd + cardOrderID + buyerAccount + urllib.quote(cardInfo.serverName.decode("gbk").encode("utf-8")).lower() + salesAccount + str(salesIP) + str(parValue) + KBEngine.globalData["pointCardWord"] )
		...
		try:
			...
			ht = urllib.request.urlopen(path + params%( cardNo, cardPwd, cardOrderID,buyerAccount, urllib.quote(cardInfo.serverName.decode("gbk").encode("utf-8")).lower(), salesAccount, salesIP, parValue, cardInfo.signKey )).read()
...
def overTimeHandle( cardInfo ):
	...
	try:
		ht = urllib.request.urlopen(path).read()
		result = ht.replace('<?xml version="1.0" encoding="utf-8"?>\r\n<string xmlns="http://www.gyyx.cn/">',"").replace('</string>',"")
		return int(result)
	except:
		return -1
...
class RechargeThread( BackgroundTask.BackgroundTask ):
	...
	def doBackgroundTask( self, mgr ):
		#读取网页字符
		#有效->关闭网页字符
		id = KBEngine.globalData["PointCardMgr"].id
		scMgr = KBEngine.entities[id]
...
class SuccessResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_1, cschannel_msgs.POINT_CARD_INFO_2%cardInfo.cardNo, cardInfo.price + csconst.SELL_POINT_CARD_YAJIN, "" )

		KBEngine.globalData["MailMgr"].send( None, cardInfo.buyerName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_3, cschannel_msgs.POINT_CARD_INFO_4, 0, "" )
		#try:
		...
		KST.g_baseAppEntity.lookupRoleBaseByName( "Role", cardInfo.salesName, Functor( self._onCardSellSuccessful, cardInfo.cardNo, cardInfo.price + csconst.SELL_POINT_CARD_YAJIN, cardInfo.buyerName, cardInfo.parValue  ) )
	...
	def _onCardSellSuccessful( self, cardNo, price, buyerName, parValue, callResult ):
		...
		KST.g_baseAppEntity.lookupRoleBaseByName( "Role", buyerName, Functor( self._onCardBuySuccessful, parValue ) )
...
class NoOrPwdFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		pointCardMgr.takeOffCard( cardInfo.cardNo )
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_5, cschannel_msgs.POINT_CARD_INFO_6%cardInfo.cardNo, 0, "" )

		KBEngine.globalData["MailMgr"].send( None, cardInfo.buyerName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_7, cschannel_msgs.POINT_CARD_INFO_8, cardInfo.price, "" )

		KST.g_baseAppEntity.lookupRoleBaseByName( "Role", cardInfo.salesName, Functor( self._onCardSellFailed, cardInfo.cardNo  ) )
...
class UsedFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		pointCardMgr.takeOffCard( cardInfo.cardNo )
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_5, cschannel_msgs.POINT_CARD_INFO_9%cardInfo.cardNo, 0, "" )
...
class AccountNotActivitiedFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		KBEngine.globalData["MailMgr"].send( None, cardInfo.buyerName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_7, cschannel_msgs.POINT_CARD_INFO_10, cardInfo.price, "" )
...
class CardLockedFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		pointCardMgr.takeOffCard( cardInfo.cardNo )
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_5, cschannel_msgs.POINT_CARD_INFO_11%cardInfo.cardNo, 0, "" )
...
class CardNotExistFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		pointCardMgr.takeOffCard( cardInfo.cardNo )
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_5, cschannel_msgs.POINT_CARD_INFO_12%cardInfo.cardNo, 0, "" )

		KBEngine.globalData["MailMgr"].send( None, cardInfo.buyerName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_7, cschannel_msgs.POINT_CARD_INFO_13, cardInfo.price, "" )
...
class CardValueFailedResult( RechargeResultHandle ):
	...
	def do( self, cardInfo, pointCardMgr):
		...
		KBEngine.globalData["MailMgr"].send( None, cardInfo.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_5, cschannel_msgs.POINT_CARD_INFO_14%cardInfo.cardNo, cardInfo.price + csconst.SELL_POINT_CARD_YAJIN, "" )
...
class PointCardDataBaseControl:
	...
	def initCards( self, mgr ):
		...
		KBEngine.executeRawDatabaseCommand( query, Functor( self._onInitCards, mgr ) )
	...
	def saveSellCard( self, card ):
		...
		KBEngine.executeRawDatabaseCommand( query, self.__onSaveSellCard )
	...
	def saveSellingCard( self, card ):
		...
		KBEngine.executeRawDatabaseCommand( query, self.__onSaveSellingCard )

	def takeOffCard( self, cardNo ):
		"""
		"""
		query = "delete from custom_PointCardsTable where sm_cardNo =  \'%s\'"%cardNo
		KBEngine.executeRawDatabaseCommand( query, self.__onTakeOffCard)
	...
	def cancelSellingState( self, cardNo ):
		...
		KBEngine.executeRawDatabaseCommand( query, self.__onCancelSellingState )


	def _onInitCards( self, mgr, result, dummy, insertid, errstr):
		"""
		"""
		if errstr:
			ERROR_MSG(errstr)
		...
		for i in result:
			...
			if time.time() - card.sellTime  > CARD_VALID_TIME:
				KBEngine.globalData["MailMgr"].send( None, card.salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_15, cschannel_msgs.POINT_CARD_INFO_16, 0, "" )
	...
	def __onSaveSellCard( self, result, dummy, insertid, errstr):
		"""
		"""
		if errstr:
			ERROR_MSG(errstr)
	...
	def __onSaveSellingCard( self, result, dummy, insertid, errstr):
		"""
		"""
		if errstr:
			ERROR_MSG(errstr)
	...
	def __onTakeOffCard( self, result, dummy, insertid, errstr):
		"""
		"""
		if errstr:
			ERROR_MSG(errstr)
	...
	def __onCancelSellingState( self, result, dummy, insertid, errstr):
		"""
		"""
		if errstr:
			ERROR_MSG(errstr)
...
class PointCardMgr( MgrPersistentObject.MgrPersistentObject ):
	#点卡寄售管理器
	def __init__(self):
		"""
		"""
	...
	def onTimer( self, id, userArg ):
		...
		if userArg == HANDLE_CARD_SELLING_CBID:
			...
			for cardNo, salesName in outDateInfos:
				KBEngine.globalData["MailMgr"].send( None, salesName, csdefine.MAIL_SENDER_TYPE_SYS, \
						cschannel_msgs.MAIL_SEND_SYSTEM, cschannel_msgs.POINT_CARD_INFO_15, cschannel_msgs.POINT_CARD_INFO_16%cardNo, 0, "" )
