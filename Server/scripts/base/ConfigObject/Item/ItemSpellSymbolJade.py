...
class ItemSpellSymbolJade(ItemSpellPosAndDirAssist.ItemSpellPosAndDirAssist):
	...
	def checkUse(self, owner):
		...
		if space.__class__.__name__ == "SpaceDongFu":
			...
			if space.getTrapNum() >= self.maxNum:
				csstatus.LHMJ_SYMBOL_JADE_HAS_EMOUGH
	...
	def useToPosAndDir(self, owner, pos, dir):
		...
		用客户端位置、朝向使用物品
		"""
		owner.createEntityByScriptID( self.scriptID, pos, dir, {"playerDBID" : owner.playerDBID, "playerID" : owner.id, "lifeTime" : time.time() + self.lifeTime} )			# 因为洞府是个 副本，出副本就销毁了！所以，直接保存entityID
