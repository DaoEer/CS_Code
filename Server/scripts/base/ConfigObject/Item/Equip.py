...
class Equip(ItemUse.ItemUse):
	...
	def getIntensifyStarLvs(self):
		"""
		获取装备配置的强化星
		"""
		return (self.goldStar, self.silverStar, self.copperStar, self.ironStar)
	...
	def isCreateAttachProperties(self):
		...
		装备是否要创建随机属性
		"""
		return self.isEquip() and (not self.isWhite()) and (self.getItemDict()["Param3"] == "")
	
	def createAttachProperties(self):
		"""
		创建非白装的随机属性
		"""
		if self.dynamicData["attachPropertiesStr"] != "":	#配置了固定属性，就不随机
			self.calculateCombatPower()
			return
