...
class PresentChargeUnite:
	...
	def onGetCell( self ):
		"""
		"""
		self.pcu_takeThings( csconst.PCU_TAKECHARGE, 0 )
	...
	def takeOverSuccess( self ):
		...
		if self._pcu_addSilverCoinTmp:
			self.addXuanShi( self._pcu_addSilverCoinTmp, csdefine.CHANGE_XUANSHI_CHARGE )	# 之前已经检查过是否能增加银元宝 所以这里不再检查。
	...
	def takeSilverCoins( self, silverCoins ):
		...
		if not self.accountEntity.testAddXuanShi( addSilverCoin ):	#表示超过了上限
			self.takeOverFailed( )
			self.statusMessage( csstatus.PCU_CAN_NOT_ADD_XUANSHI )
			return
		self.addXuanShi( addSilverCoin, csdefine.CHANGE_XUANSHI_SILVERPRESENT )
	...
	def takeChargedMoney( self, silverCoinsList, goldList ):
		...
		if not self.accountEntity.testAddXuanShi( m_addSilverCoin ):	#表示超过了上限
			self.takeOverFailed( )
			self.statusMessage( csstatus.PCU_CAN_NOT_ADD_XUANSHI )
			return
		for gold in goldList:
			m_addGold += gold
		if not self.accountEntity.testAddXianShi( m_addGold ):	#表示超过了上限
			self.takeOverFailed( )
			self.statusMessage( csstatus.PCU_CAN_NOT_ADD_XIANSHI )
			return
		if m_addGold > 0:
			self.addXianShi( m_addGold, csdefine.CHANGE_XIANSHI_CHARGE )
		if m_addSilverCoin > 0:
			self.addXuanShi( m_addSilverCoin, csdefine.CHANGE_XUANSHI_CHARGE )
		self.takeOverSuccess()					#通知清理数据
	...
	def takePresents( self, presentIDs, silverCoins ):
		...
		if not self.accountEntity.testAddXuanShi( self._pcu_addSilverCoinTmp ):	#这里检查是否能够增加银元宝
			self.takeOverFailed( )
			self.statusMessage( csstatus.PCU_CAN_NOT_ADD_XUANSHI )
			return
