...
class MonsterOwnershipFirstDamage(MonsterOwnershipBase.MonsterOwnershipBase):
	"""
	"""
	...
	def canAddOwnerCheck(self, monsterEntity, ownerEntity):
		"""
		"""
		#判断怪物有没有归属者，如果已经有归属者了不能再设置归属者
		if monsterEntity.hasBootOwner():
			return False
	...
	def onRemoveEnemy(self, monsterEntity, ownerEntityID):
		...
		if not entity:
			...
			#如果玩家或者玩家玩家队伍没有人在怪物的敌人列表中 移除归属权
			for oid in ownerIDs:
