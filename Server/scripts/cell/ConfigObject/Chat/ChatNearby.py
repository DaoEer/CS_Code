...
class ChatNearby( ChatBase ):
	...
	def verify( self, avatar, chatData ):
		...
		验证发送
		"""
		return ChatBase.verify( self, avatar, csdefine.CHAT_TYPE_NEARBY, chatData )
	...
	def send( self, avatar, chatData ):
		...
		avatar.chatTime[csdefine.CHAT_TYPE_NEARBY] = time.time()
		avatar.chatNum[csdefine.CHAT_TYPE_NEARBY] += 1
		playerList = avatar.entitiesInRangeExt( csdefine.CHAT_NEARBY_RANGE, "Role" )
		for player in playerList:
			if not player.cellHadBlacklist(avatar.playerDBID):
