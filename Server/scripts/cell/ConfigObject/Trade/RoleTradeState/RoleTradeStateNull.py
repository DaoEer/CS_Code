...
class RoleTradeStateNull( RoleTradeStateBase ):
	"""
	玩家没有交易状态
	"""
	...
	def changeState( self, state, srcEntity, dstEntity ):
		...
		"""
		if state == TRADE_STATE_INVITE:
			if dstEntity.getCurrTradeState() == TRADE_STATE_NULL:
				return True

		if state == TRADE_STATE_WAIT:
			return True			
	...
	def onDstStateChanged( self, srcEntity, dstEntity, state ):
		...
		if state == TRADE_STATE_INVITE:
			# 对方的状态转变为邀请状态
			srcEntity.changeRoleTradeState( TRADE_STATE_WAIT, dstEntity )
			return
