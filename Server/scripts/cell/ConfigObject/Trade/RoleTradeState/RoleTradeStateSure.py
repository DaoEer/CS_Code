...
class RoleTradeStateSure( RoleTradeStateBase ):
	"""
	玩家交易确认状态
	"""
	...
	def enter( self, srcEntity, dstEntity ):
		...
		"""
		srcEntity.roleTradeState = TRADE_STATE_SURE
	...
	def onDstStateChanged( self, srcEntity, dstEntity, state ):
		...
		if state == TRADE_STATE_SURE:
			if srcEntity.getCurrTradeState() == TRADE_STATE_SURE:
				payItem = {TRADE_PAY_ITEM_ROLE_TRADE : None}
				srcEntity.requestPay( TRADE_TYPE_ROLE, dstEntity.id, payItem )
		if state == TRADE_STATE_NULL:
