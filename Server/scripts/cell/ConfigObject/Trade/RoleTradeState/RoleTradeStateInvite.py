...
class RoleTradeStateInvite( RoleTradeStateBase ):
	"""
	玩家没有交易状态
	"""
	...
	def enter( self, srcEntity, dstEntity ):
		...
		"""
		srcEntity.roleTradeState = TRADE_STATE_INVITE

		srcEntity.anotherRoleTradeEntity = dstEntity
		dstEntity.onRoleTrade( srcEntity )
		srcEntity.roleTradeOvertimeID = srcEntity.addTimerCallBack( TRADE_OVERTIME_LIMIT, "requestRoleTradeOvertime", () )
	...
	def changeState( self, state, srcEntity, dstEntity ):
		...
		if state == TRADE_STATE_INVITE:		# 在邀请状态还可以变更交易对象
			return True
