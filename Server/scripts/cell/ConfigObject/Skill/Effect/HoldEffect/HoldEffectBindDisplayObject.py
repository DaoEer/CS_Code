...
class HoldEffectBindDisplayObject( HoldEffect ):
	"""
	持有效果：模型绑定部件
	"""
	def __init__( self ):
		HoldEffect.__init__( self )
		self._meshID = ""						# 客户端HoldPetMeshTable表id
	...
	def onBegin( self, effectData, receiver ):
		...
		receiver.allClients.CLIENT_BindDisplayObject(self._meshID)

	def onEnd( self, effectData, receiver ):
		"""
		效果结束
		"""
		receiver.allClients.CLIENT_UnbindDisplayObject(self._meshID)
