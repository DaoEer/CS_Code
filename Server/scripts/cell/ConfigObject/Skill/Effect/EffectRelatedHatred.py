...
class EffectRelatedHatred( EffectSkillBase ):
	"""
	连带仇恨改变效果
	"""
	def __init__( self ):
	...
	def onReceive( self, skill, caster, receiver ):
		...
		效果开始
		"""
		for enemyID in receiver.enemyList.keys():
			enemy = KBEngine.entities.get(enemyID, None)
			if enemy:
				hatredValue = self.getHatredValue(caster, receiver, enemy)
				enemy.addHatredList(caster.id, hatredValue)

	def getHatredValue(self, caster, receiver, enemy):
		hatredValue = 0
		if self._effectType == 1:
			casterBaseCom = g_CombatNormalDamager.getBaseCombatData( caster.getLevel(), caster.getProfession() )
			hatredValue = math.ceil(casterBaseCom["BRelatedHatred"] * self._percent + self._value)
		elif self._effectType == 2:
			receiverBaseCom = g_CombatNormalDamager.getBaseCombatData( receiver.getLevel(), receiver.getProfession() )
			hatredValue = math.ceil(receiverBaseCom["BRelatedHatred"] * self._percent + self._value)
		elif self._effectType == 3:
			BaseCom = g_CombatNormalDamager.getBaseCombatData( enemy.getLevel(), enemy.getProfession())
			hatredValue = math.ceil(BaseCom["BRelatedHatred"] * self._percent + self._value)
		return hatredValue	
