...
class MonsterExtQuest( Monster.Monster ):
	...
	def initQuestStatuesDict(self, questStatus):
		...
		for status in _questStatus:
			...
			qs = strList[0].split(":")
			questID = int(qs[0])
			self.questStatusDict[questID] = {"questStatus":int(qs[1]),"taskStatus":{}}
			if len(strList)<2:	continue
			for taskStatus in strList[1:]:
				ts = taskStatus.split(":")
				self.questStatusDict[questID]["taskStatus"].update({int(ts[0]):int(ts[1])})
	...
	def checkQuestStatues(self, playerEntity):
		...
		for questID, statusValue in self.questStatusDict.items():
			if not playerEntity.hasQuest(questID):
				continue
			if playerEntity.queryQuestStatus(questID) != statusValue["questStatus"]:
				continue
			result = True
			for taskID, taskStatus in statusValue["taskStatus"].items():
