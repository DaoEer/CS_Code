...
class Line( object ) :
	...
	def isSeamIntersectant( self, line ) :
		...
		"""
		point = self.getIntersectantPoint( line )
	...
	def getSeamIntersectantPoint( self, line ) :
		...
		if point is None :
			return None
		if self.isInnerPoint( point ) and line.isInnerPoint( point ) :
			return point
		return None
...
class Rect( object ) :
	...
	def getRandomPoint(self):
		...
		获得距形内的随机点
		"""
		return (self.__x + random.uniform(0, self.__width), self.__y + random.uniform(0, self.__height))
...
class Polygon( object ) :
	...
	def getRandomPoint(self):
		...
		获得多边形内的随机点
		"""
		point = self.__bound.getRandomPoint()
